...
module IO =
    ...
    let rec printLines l =
        ...
        | [] -> printfn ""
        | [head] -> 
            printfn ">SE%d_G%d\n%s" head.speciesId head.geneId (toString head.dna)
        | head::tail -> 
            printfn ">SE%d_G%d\n%s" head.speciesId head.geneId (toString head.dna)
            printLines tail

let input = IO.readLines @"/Users/alex/Repository/Evolution/Tests/test10" 
            |> Seq.toList
            |> List.map IO.split

let output = List.fold (fun acc evt -> DNA.events acc evt) ([]: DNA.DNA list) input
            |> List.sort
            |> IO.writeLines @"/Users/alex/Repository/Evolution/Results/test_v1.fa"
